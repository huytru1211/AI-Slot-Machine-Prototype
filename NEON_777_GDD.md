...
|---|---|---|
| RTP (base + free spins) | 94–97% | **~95.66%** (node sim result) |
| Hit Frequency | 28–34% | **~32.29%** |
| House Edge | 3–6% | ~4.34% |

The simulation function `simulate(n)` runs n headless spins through the actual engine, returning `{ rtp, hitFreq, freeSpinRate, avgWin, maxWin, spins, freeSpinTriggers }`.

- In-browser: runs 200,000 spins after boot, displayed in the ODDS tab of the Info panel.
- Dev tool (`node tools/sim.mjs`): runs 1,000,000 spins in Node.js.
- **hitFreq formula:** `hits / (n + totalFsSpins)` — `totalFsSpins` is a counter incremented inside the free-spin while-loop (tracks every actual FS spin played, including retrigger bonus rounds). Earlier versions used `freeSpinTriggers × count` which undercounted the denominator when retriggers added bonus spins, inflating hitFreq by ~0.1–0.3%.
...
const CONFIG = {
  ...
  paytable:   { '7':75, wild3:38, BAR:15, '🔔':8, '🍋':5, cherry3:3, cherry2:1, scatterPay:2 },
